...
- Web UI (`data/index.html`) consumes status/config and renders hero cards, status labels, curves, and controls.
- Mock server (`tools/mock_otthing.py`) emulates firmware endpoints and should match runtime shape closely.
...
- Use dotted status paths exactly as produced by firmware (`heatercircuit.0.roomsetpoint`, etc.).
- Treat status/config names as case-sensitive and canonical.
- If mock fields differ from firmware names, fix mock to firmware names (not vice versa) unless explicitly requested.
...
- If adding mock controls, ensure:
  ...
  - `state["status"]` seed contains representative values.
  - top quick editor, card controls, and JSON viewer stay in sync after updates.
- If changing value coloring in `data/index.html`, preserve explicit rule: no valid range => no color.
...
- `tools/mock_otthing.py`
  ...
  - JSON viewer interactions should support selecting a path for quick editing.
  - `sendValue` success path should refresh controls and JSON boxes coherently.
- `src/` and `include/`
  - Use these files to confirm field names and bit/flag semantics before changing UI or mock fields.

## Common pitfalls
- Adding wrong nested objects in mock status (for example `roomComp.{...}`) when firmware uses flat fields (`roomcompInteg`).
- Updating only mock controls without adding default seed values in `state["status"]`.
- Updating quick editor behavior without syncing card controls or JSON viewer.
...
2. New/updated mock field is editable via card controls and top quick path editor.
3. JSON viewer updates after value changes.
4. If graph behavior is affected, confirm required config and status keys exist.
5. If naming was touched, verify canonical spelling is used consistently.

## Style
- Keep code and text ASCII unless file already uses Unicode.
- Keep comments short and only where non-obvious.
- Preserve existing formatting and structure.
- Prefer smallest safe patch over broad reformatting.